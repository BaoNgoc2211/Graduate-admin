...
export function PurchaseOrderItemRow({
  ...
  const { data: batchData } = useImportBatch();
  const batchMedicines: IImportBatch[] = batchData?.data || [];

  const { setValue } = useFormContext<PurchaseOrderFormData>();
            ...
            <FormItem className="w-[150px]">
              ...
              <Select
                onValueChange={(value) => {
                  ...
                  const medicine = medicines.find((m) => m._id === value);
                  if (medicine) {
                    setValue(`medicines.${index}.price`, medicine.price);
                  }
                }}
                value={field.value}
                disabled={isLoading}
          ...
          render={({ field }) => (
            <FormItem>
              ...
              >
                ...
                <SelectContent>
                  <SelectItem value="none">Không chọn lô</SelectItem>
                  {batchMedicines.map((batch) => (
                    <SelectItem key={batch._id} value={batch._id}>
                      <div className="flex flex-col">
                        <span className="font-medium">{batch.batchNumber}</span>
