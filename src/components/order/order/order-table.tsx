...
export default function OrderTable({
  ...
  const getStatusIcon = (status: IOrder["status"]) => {
    switch (status) {
      case "Chờ xác nhận": // ✅ Sửa: Đổi từ "Pending Confirmation" thành "Chờ xác nhận"
        return <Clock className="h-4 w-4" />;
      case "Chờ giao hàng": // ✅ Sửa: Đổi từ "Awaiting Shipment" thành "Chờ giao hàng"
        return <Package className="h-4 w-4" />;
      case "Đang giao": // ✅ Sửa: Đổi từ "Shipping" thành "Đang giao"
        return <Truck className="h-4 w-4" />;
      case "Hoàn thành": // ✅ Sửa: Đổi từ "Completed" thành "Hoàn thành"
        return <CheckCircle className="h-4 w-4" />;
      case "Đã hủy": // ✅ Sửa: Đổi từ "Cancelled" thành "Đã hủy"
        return <XCircle className="h-4 w-4" />;
      default:
        return null;
    }
  ...
  const getNextStatus = (
    ...
    switch (currentStatus) {
      case "Chờ xác nhận": // PENDING
        return "Chờ giao hàng"; // CONFIRMED
      case "Chờ giao hàng": // CONFIRMED
        return "Đang giao"; // DELIVERING
