...
// }
export const getAllImportBatchAPI = async (
  page: number = 1,
  pageSize: number = 20
): Promise<{ data: IImportBatch[] }> => {
...
export const updateImportBatchAPI = async (
  batch_id: string,
  payload: IImportBatchFormData
): Promise<IImportBatch> => {
  const res = await APIConfig.put(
    `/api/import-batch/update-import-batch/${batch_id}`,
    payload
  );
  return res.data;
};
...
export const updateImportBatchStatusAPI = async (
  ...
  const res = await APIConfig.put(
    `/api/import-batch/update-status/${batch_id}`,
    { status }
  );
  return res.data;
};
