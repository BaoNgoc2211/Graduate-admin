 ...
 */
export const reviewOrderAPI = async (payload: ICheckoutPayload): Promise<IOrderReview> => {
  const response = await APIConfig.post("/api/order/review", payload)
  return response.data.data as IOrderReview
}

/**
 * API thực hiện đặt hàng
 */
export const checkoutAPI = async (payload: ICheckoutPayload): Promise<IOrderReview> => {
  const response = await APIConfig.post("/api/order/review/checkout", payload)
  return response.data.data as IOrderReview
}
