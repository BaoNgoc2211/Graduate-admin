...
export const purchaseOrderSchema = z.object({
  ...
  date_in: z
    .date({ required_error: "Vui lòng chọn ngày tạo đơn" })
    .refine((date) => date <= new Date(), "Ngày không được trong tương lai"),
  expectedDeliveryDate: z.date().optional(),
  note: z.enum(["Nháp", "Ghi nợ", "Đã thanh toán"]).optional(),
  medicines: z
    .array(
      z.object({
        medicine_id: z.string().min(1, "Vui lòng chọn thuốc"),
        ...
        price: z
          ...
          .min(0, "Đơn giá không thể âm")
          .max(1000000000, "Đơn giá không được quá 1 tỷ đồng"),
        // ✅ Thêm .optional() để có thể là undefined
        CK_Rate: z.number().min(0).max(100).optional(),
        VAT_Rate: z.number().min(0).max(100).optional(),
        batch_id: z.string().optional().or(z.literal("")).or(z.literal("none")),
      })
    )
    .min(1, "Phải có ít nhất 1 thuốc"),
  totalAmount: z
...
//     .refine((date) => date <= new Date(), "Ngày không được trong tương lai"),
//   expectedDeliveryDate: z.date().optional(),
//   note: z.enum(["Nháp", "Ghi nợ", "Đã thanh toán"]).default("Nháp"),
//   medicines: z
//     .array(
//       z.object({
//         medicine_id: z.string().min(1, "Vui lòng chọn thuốc"),
