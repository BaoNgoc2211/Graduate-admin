...
import { IPurchaseEntry, IPurchaseFilter } from "@/interface/order/purchase.interface"
import { usePurchaseStats } from "@/hooks/order/purchase.hooks"
import { PurchaseForm } from "@/components/order/purchase/form"
import { formatPrice } from "@/lib/format-price"
import { FilterPanel } from "@/components/order/purchase/filter-pane"
import { PurchaseTable } from "@/components/order/purchase/table"
import { StatCard } from "@/components/order/purchase/purchase-statcard"


export default function PurchasePage() {
  const [filters, setFilters] = useState<IPurchaseFilter>({})
  const [showForm, setShowForm] = useState(false)
  const [editingEntry, setEditingEntry] = useState<IPurchaseEntry | null>(null)
  const [viewingEntry, setViewingEntry] = useState<IPurchaseEntry | null>(null)
  const { data: statsData, isLoading: statsLoading } = usePurchaseStats()
  const stats = statsData?.data

  const handleFiltersChange = (newFilters: IPurchaseFilter) => {
  ...
  const handleEdit = (entry: IPurchaseEntry) => {
    setEditingEntry(entry)
    setShowForm(true)
  }

  const handleView = (entry: IPurchaseEntry) => {
    ...
    return (
      <div className="min-h-screen bg-white">
        <div className="container mx-auto px-4 py-6">
          <PurchaseForm entry={editingEntry || undefined} onSuccess={handleFormSuccess} onCancel={handleFormCancel} />
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="container mx-auto px-4 py-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-blue-900">Quản lý phiếu nhập kho</h1>
              <p className="text-gray-600 mt-1">Quản lý các phiếu nhập kho và theo dõi tình trạng duyệt</p>
            </div>
            <Button onClick={handleCreateNew} className="bg-blue-900 hover:bg-blue-800 flex items-center gap-2">
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <StatCard title="Tổng số phiếu" value={stats?.totalEntries || 0} icon={FileText} loading={statsLoading} />
          <StatCard
            title="Tổng giá trị"
          ...
          />
          <StatCard title="Chờ duyệt" value={stats?.pendingEntries || 0} icon={Clock} loading={statsLoading} />
          <StatCard title="Tháng này" value={stats?.thisMonthEntries || 0} icon={CheckCircle} loading={statsLoading} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1">
            <FilterPanel filters={filters} onFiltersChange={handleFiltersChange} onClearFilters={handleClearFilters} />
          </div>

          {/* Table */}
          <div className="lg:col-span-3">
            <PurchaseTable filters={filters} onEdit={handleEdit} onView={handleView} />
          </div>
        </div>
      </div>
    </div>
