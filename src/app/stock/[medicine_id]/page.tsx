...
//             <Tabs
//               value={activeTab}
//               onValueChange={(v) => setActiveTab(v as TabType)} // ✅ FIX: Use proper type assertion
//             >
...
const StockDetailPage: React.FC = () => {
  ...
  // FIX: Gọi tất cả hooks ở đầu component, không có điều kiện
  const { data, isLoading, isError, refetch } = useByMedicineStock(medicineId);

  // Get stocks data trước khi tính stats
  ...
  // const handleMedicineUpdate = (medicineId: string, updatedData: IMedicineUpdate) => {
  //   console.log("Updating medicine:", medicineId, updatedData);
  //   refetch();
  // };
  ...
  const handleBatchSelectionChange = (selectedBatchIds: string[]) => {
    ...
    // This could trigger API calls to update which batches are displayed
    console.log("Selected batch IDs:", selectedBatchIds);
    // You might want to call an API to update the displayed batches
